      ...
      try:
        ...
        try:
          sent_message = message.reply_text(
              " {} , আপনি আমাদের অফিসিয়াল টেলিগ্রাম চ্যানেলটিতে JOIN করেননি তাই আপনাকে Mute করে দেওয়া হয়েছে, গ্রুপে কথা বলার জন্য আপনাকে Subscribe Our Channel বাটনটিতে ক্লিক করে আমাদের অফিসিয়াল চ্যানেলটি Subscribe করে আসার পর নিচের Unmute Me বাটনটিতে ক্লিক করলেই আপনি গ্রুপে কথা বলতে পারবেন। Help @Root_Of_Cyber".format(message.from_user.mention, channel, channel),
...
@Client.on_message(filters.command(["rocpromoterbot", "rocx", "rocx@roc_promoter_bot", "roc_promoter_bot@roc_promoter_bot"]) & ~filters.private)
  ...
  if user.status is "creator" or user.user.id in Config.SUDO_USERS:
    ...
    if len(message.command) > 1:
      ...
      if input_str.lower() in ("off", "no", "disable"):
        ...
        message.reply_text("❌ **roc promoter bot is Disabled Successfully.**")
        ...
        try:
          ...
          message.reply_text(f"✅ **roc promoter bot is Enabled**\n__roc promoter bot is enabled, all the group members have to subscribe this [channel]({channel_url}) in order to send messages in this group.__", disable_web_page_preview=True)
        except UserNotParticipant:
          message.reply_text(f"❗ **Not an Admin in the Channel**\n__I am not an admin in the [channel]({channel_url}). Add me as a admin in order to enable roc promoter bote.__", disable_web_page_preview=True)
      ...
      if sql.fs_settings(chat_id):
        ...
        message.reply_text(f"✅ **roc promoter bot is enabled in this chat.**\n__For this [Channel]({channel_url})__", disable_web_page_preview=True)
      else:
        message.reply_text("❌ **roc promoter bot is disabled in this chat.**")
  else:
